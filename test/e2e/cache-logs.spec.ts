...
const LOG_FILE = process.env.RARI_LOG_FILE ?? './target/rari-web.log'
...
test('response cache: invalidate_by_tag clears the entry', async ({ request }) => {
  ...
  const aboutHits = grepLog(/memory cache hit.*\/about|\/about.*memory cache hit/)
  const keyHits = grepLog(/memory cache hit/)
  expect(keyHits.length).toBeGreaterThan(aboutHits.length) // sanity
})
...
test('fetch cache: GET /fetch-test twice - first miss, second hit', async ({ page }) => {
  ...
  const fetchCachePopulated = /set_with_tags\s+key=\d{10,}/.test(plain)
...
test('log file: contains both boot logs and request logs', async () => {
  ...
  expect(content).toMatch(/memory cache handler initialized/)
  const expected = [
    /memory cache miss/,
  ...
  if (missing.length > 0) {
    throw new Error(
      `Missing log patterns: ${missing.map(r => r.toString()).join(', ')}\n`
      + `--- last 4KB ---\n${content.slice(-4096)}`,
    )
  }
})
